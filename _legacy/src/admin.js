...
function syncDatabaseFromCloud(callback) {
  fetch('https://kvdb.io/ptkv_placements_8f3d2a7c/users_database')
    .then(res => res.json())
    .then(cloudDb => {
      if (Array.isArray(cloudDb)) {
        localStorage.setItem('users_database', JSON.stringify(cloudDb));
        if (callback) callback(cloudDb);
      }
    })
    .catch(err => {
      console.error("Cloud fetch failed, falling back to local storage:", err);
      const localDb = getDatabase();
      if (callback) callback(localDb);
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load initial database from cloud
  syncDatabaseFromCloud((db) => {
    renderDashboard(db);
    window.lastDbSerialized = JSON.stringify(db);
  });

  // Set up live polling to automatically refresh registered list and live online count from cloud
  setInterval(() => {
    syncDatabaseFromCloud((freshDb) => {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ...
      const localDb = getDatabase();
      const filtered = localDb.filter(user => {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      if (confirm('Are you sure you want to reset the user database back to defaults? All custom registrations will be cleared.')) {
        ...
        const defaultDb = defaultMockUsers;
        localStorage.setItem('users_database', JSON.stringify(defaultDb));
        renderDashboard(defaultDb);
        if (searchInput) searchInput.value = '';

        // Sync clear/reset to cloud
        fetch('https://kvdb.io/ptkv_placements_8f3d2a7c/users_database', {
          method: 'PUT',
